...
class Widgets:
    ...
    def __init__(self, window, canvas):
        ...
        self._start_b = tk.Button(self._window, text='Start',
                                  font='Courier 15', command=self._start)
        self._score_l = tk.Label(self._window, textvariable=self._score_var,
                                 font='Courier 30')
    ...
    def new_green_rectangle(self):
        ...
        self._canvas.create_rectangle(0, Intro.CH - Widgets._RECT_HEIGHT,
                                      Intro.CW, Intro.CH, fill='lightgreen',
                                      outline='lightgreen')
...
class Pipes:
    ...
    def __init__(self, window, canvas):
        ...
        self._top_pipe_y2 = Intro.CH / 2 - Pipes._GAP_SIZE / 2
        self._botom_pipe_y1 = Intro.CH / 2 + Pipes._GAP_SIZE / 2

        self._pipes = [[self._first_pipes(Intro.CW, 0, Intro.CW +
                                          Pipes._PIPE_W, self._top_pipe_y2),
                       self._first_pipes(Intro.CW, self._botom_pipe_y1,
                                         Intro.CW + Pipes._PIPE_W, Intro.CH),
                        True],
                       [self._first_pipes(1.5 * Intro.CW, 0, 1.5 * Intro.CW +
                                          Pipes._PIPE_W, self._top_pipe_y2),
                       self._first_pipes(1.5 * Intro.CW, self._botom_pipe_y1,
                                         1.5 * Intro.CW + Pipes._PIPE_W,
                                         Intro.CH), True],
                       [self._first_pipes(2 * Intro.CW, 0, 2 * Intro.CW +
                                          Pipes._PIPE_W, self._top_pipe_y2),
                       self._first_pipes(2 * Intro.CW, self._botom_pipe_y1,
                                         2 * Intro.CW + Pipes._PIPE_W,
                                         Intro.CH),
                        True]]
    ...
    def _move_pipe(self):
        ...
        if self._game_over is False:
            for i in range(len(self._pipes)):
                if self._canvas.coords(self._pipes[i][0])[Play.X1] <= \
                        -Intro.CW / 4:
    ...
    def _create_new_pipes(self, pipe):
        ...
        return [self._canvas.create_rectangle(1.25 * Intro.CW, 0, 1.25 *
                                              Intro.CW + Pipes._PIPE_W,
                                              pipe_height[0], outline='green',
                                              fill='lightgreen',
                                              width=Play.BORDER),
                self._canvas.create_rectangle(1.25 * Intro.CW, pipe_height[1],
                                              1.25 * Intro.CW + Pipes._PIPE_W,
                                              Intro.CH, width=Play.BORDER,
                                              fill='lightgreen',
                                              outline='green'), True]

    def _first_pipes(self, *args, ):
        """Help create 1st 3 sets of pipes with the parameters."""
        return self._canvas.create_rectangle(*args, width=Play.BORDER,
                                             outline='green',
                                             fill='lightgreen')
...
class Play:
    ...
    def __init__(self, window, canvas):
        """Act as the main part of the program."""
        self._game_over = False
        self._not_play = True
        self._pipes = None
        ...
        self._player = self._canvas.create_rectangle(0.25 * Intro.CW -
                                                     Play._BIRD_R, Intro.CH / 2
                                                     - Play._BIRD_R, 0.25 *
                                                     Intro.CW + Play._BIRD_R,
                                                     Intro.CH / 2 +
                                                     Play._BIRD_R, fill='red',
                                                     width=Play.BORDER,
                                                     outline='orange')
        self._p_speed = 0
    ...
    def _summary(self):
        ...
        self._widgets.new_text(Intro.CW / 2, Intro.CH / 2 - Intro.TEXT_D,
                               text='GAME OVER', fill='blanchedalmond',
...
class Intro:
    ...
    _INTRO = 'Instructions: To play press start. Once you have ' \
             'clicked space or left button on the mouse,\n the bird ' \
             'moves up and the pipes start to come. The aim of the ' \
             'game is to get through\nas many pipes as possible ' \
    ...
    def __init__(self):
        ...
        self._canvas = tk.Canvas(self._window, height=Intro.CH, width=Intro.CW,
                                 bg='lightblue')
        self._widgets = Widgets(self._window, self._canvas)
        ...
        self._widgets.new_text(Intro.CW / 2, Intro.CH / 4 + Intro.TEXT_D,
                               ...
                               text=Intro._INTRO)
        self._widgets.new_text(Intro.CW / 2, Intro.CH / 4 - Intro.TEXT_D,
                               font='Courier 30 bold', fill='blanchedalmond',
                               text=Intro._WELCOME_TEXT)
    ...
    def __init__(self):
        ...
        while self._play_again != Menu._NO:
            ...
            while self._play_again not in [Menu._NO, Menu._YES]:
                self._play_again = input("Would you like to play again? "
